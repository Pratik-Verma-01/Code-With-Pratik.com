...
export default function AuthForms() {
  ...
  const onLogin = async (data) => {
    ...
    try {
      ...
      // Toast is handled in AuthContext or we can do it here
      navigate('/dashboard');
    } catch (error) {
      console.error(error);
      ...
      >
        ...
        <div className="mt-8 relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-[#1e293b] text-gray-400 bg-opacity-80 backdrop-blur">
              Or continue with
            </span>
          </div>
        </div>
        ...
        <button 
          ...
          onClick={onGoogle}
          disabled={isLoading}
          className="mt-4 w-full flex items-center justify-center gap-2 bg-white text-black font-medium py-3 rounded-xl hover:bg-gray-100 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {isLoading ? (
